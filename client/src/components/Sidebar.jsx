  ...
  const deleteChat = async (e, chatId) => {
    try {
      ...
      const confirm = window.confirm(
        "Are you sure you wnat to delete this chat?",
      );
      if (!confirm) return;
  ...
  return (
    ...
    >
      ...
      <button
        ...
        className="flex justify-center items-center w-full py-2 mt-5 text-white bg-gradient-to-r from-[#A456F7] to-[#3D81F6] text-sm rounded-md cursor-pointer"
      >
      ...
      {/* Dark Mode Toggle */}
      <div className="flex items-center justify-between gap-2 p-3 mt-1.5 border border-gray-300 dark:border-white/15 rounded-md">
      ...
      {/* User Account */}
      <div className="flex items-center justify-between gap-3 p-3 mt-3 border border-gray-300 dark:border-white/15 rounded-md cursor-pointer group">
        ...
        {user && (
          ...
          <img
            onClick={logout}
            src={assets.logout_icon}
            className="h-5 cursor-pointer block md:hidden md:group-hover:block not-dark:invert"
          />
        )}
      </div>
